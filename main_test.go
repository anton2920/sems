...
import (
	...
	myurl "github.com/anton2920/gofa/net/url"
)
...
func testGet(t *testing.T, endpoint string, expectedStatus http.Status) {
	...
	r.URL.Path = myurl.Path(endpoint)
	...
	if w.Status != expectedStatus {
		t.Errorf("GET %s -> %d, expected %d", endpoint, w.Status, expectedStatus)
	}
}
...
func testPost(t *testing.T, endpoint string, form url.Values, expectedStatus http.Status) {
	...
	if w.Status != expectedStatus {
		t.Errorf("POST %s -> %d (with form %v), expected %d", endpoint, w.Status, form, expectedStatus)
	}
}
...
func testPostInvalidFormAuth(t *testing.T, endpoint string, token string) {
	...
	if w.Status != http.StatusBadRequest {
		t.Errorf("POST %s -> %d (with invalid payload), expected 400", endpoint, w.Status)
	}
}
