/* NOTE(anton2920): this is the backport from internal/bytealg. Unfortunately, current server does not support AVX2, so it's relying only on SSE. */
...
TEXT ·FindSubstring(SB),NOSPLIT,$0-40
	MOVQ a_base+0(FP), DI
	MOVQ a_len+8(FP), DX
...
TEXT	findcharbody<>(SB), NOSPLIT, $0
...
TEXT findsubstringbody<>(SB),NOSPLIT,$0
	CMPQ AX, DX
	JA fail
	CMPQ DX, $16
	JAE sse42
